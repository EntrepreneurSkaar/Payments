...
import {
  ChevronDown, CalendarDays, Clock, Download, Printer, Settings2,
} from 'lucide-react'
...
export default function PayoutReconciliationPage() {
  ...
  // Summary numbers (simplified MVP)
  const startingBalance = 0
  const payouts = 0 // set to >0 later when you simulate payouts in your data
  const endingBalance = startingBalance + netChangeFromActivity - payouts

  // Rows that explain what made up payouts in this time range
  const payoutRows: ActivityRow[] = useMemo(() => ([
    ...
    <main className="min-h-screen bg-white">
      <div className="max-w-8xl mx-auto py-8 space-y-8">
        ...
        <div className="text-sm text-zinc-500">
          ...
          <span className="text-zinc-700">Payout reconciliation</span>
        </div>
        ...
        <div className="flex flex-col gap-4">
          ...
          <p className="text-sm text-zinc-600">
            Payouts made to your bank account for this period, based on the <span className="underline underline-offset-2">balance change date</span>.
            Useful to reconcile payouts against your bank statements.
          </p>
        ...
        {/* Card: Balance summary — Platform (quick recap at top) */}
        <section className="rounded-xloverflow-hidden">
          <div className="flex items-start justify-between p-4 sm:p-5 border-b border-zinc-200 bg-zinc-50/50">
            <div>
              ...
              <p className="mt-1 text-sm text-zinc-600 max-w-3xl">
                Shows starting and ending balance. Changes come from activity (see Balance) and from payouts to your bank (detailed below).
              </p>
            </div>
        ...
        {/* Card: Payout reconciliation */}
        <section className="rounded-xl overflow-hidden">
          <div className="flex items-start justify-between p-4 sm:p-5 border-b border-zinc-200 bg-zinc-50/50">
            <div>
              <h2 className="text-base font-semibold text-zinc-900">Payout reconciliation</h2>
              <p className="mt-1 text-sm text-zinc-600 max-w-3xl">
                Detailed breakdown of the “Total payouts” line from the Balance Summary above. Includes
                the activity that makes up payouts in this time range, plus itemized downloads.
              </p>
            </div>
            <div className="flex items-center gap-2">
              <GhostButton icon={<Download className="w-4 h-4" />}>Download</GhostButton>
...
/* ---------- Small UI primitives (same style as Balance Summary) ---------- */
